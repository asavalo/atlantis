...
async def generate_text(
    ...
    content, usage = await a_chat_completions_create(
...
def create_openai(opt: dict):
    ...
    so that you can call, for example, `chat_completions_create(...)`.
    """
    ...
    class OpenAIClientWrapper:
        ...
        @property
        def chat(self):
            # Return self to allow chaining: chat_completions_create(...)
            return self
            ...
            if LLM_PROVIDER == "litellm" or LLM_PROVIDER == "openai":
                try:
                    if "claude" in kwargs.get("model"):
                        ...
                        kwargs["max_tokens"] = get_max_tokens(kwargs.get("model"))
                        res = await a_chat_completions_create(**kwargs)
                except Exception as e:
