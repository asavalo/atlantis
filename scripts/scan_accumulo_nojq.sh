...
for o in data:
  if isinstance(o, dict):
    if "path" not in o: o=dict({"path":path}, **o)
    fixed.append(o)
# write per-file array
with open(pff,'w',encoding='utf-8') as f:
  json.dump(fixed, f, ensure_ascii=False)
# append JSONL lines to stdout for caller to redirect
for o in fixed:
  print(json.dumps(o, ensure_ascii=False))
PY >> "$out_jsonl"

    # Count lines added (best-effort)
    added=$(grep -c '' "$out_jsonl" 2>/dev/null || echo 0)
    printf "    ↳ (w%u so far JSONL lines: %s) → %s\n" "$idx" "$added" "$PFF"
    sleep "$JITTER"
  done < "$chunk"
