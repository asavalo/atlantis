#!/usr/bin/env bash
# Adaptive parallel CRS scanner for g6e.12xlarge (4× L40S).
# - 4 workers (default), one per GPU
# - Strict JSON, retries, longer timeouts
# - On HTTP 500: halve caps, then quarter + slice file (first half, then second)
# - Per-file findings under WORKDIR/vuln_by_file/<safe>.json
# - Final merged pretty JSON at <out.json>
...
PORT_OFFSET=${PORT_OFFSET:-0}                      # shift ports if 8000/11434 busy
MODEL_NAME=${MODEL_NAME:-"llama3.1:8b-instruct-q4_K_M"}

BASE_MAX_BYTES=${MAX_BYTES_PER_FILE:-120000}       # base per-file byte cap (numbered lines)
BASE_MAX_TOKENS=${LLM_MAX_TOKENS:-384}
TEMP=${LLM_TEMPERATURE:-0}
TIMEOUT=${TIMEOUT:-1200}                           # curl max time per request (s)
RETRIES=${RETRIES:-3}
JITTER=${JITTER:-0.1}

NETWORK=${NETWORK:-"atl-net"}
ATLAS_IMG=${ATLAS_IMG:-"asavalo-atlantis-webservice"}  # your webservice image tag
...
start_worker() {
  ...
  docker ps --format '{{.Names}}' | grep -qx "ollama-$i" || docker run -d --rm \
    --gpus "device=$i" --name "ollama-$i" --network "${NETWORK}" \
    -p "127.0.0.1:${OPORT}:11434" -e OLLAMA_NUM_GPU=1 \
    ollama/ollama:latest >/dev/null

  docker ps --format '{{.Names}}' | grep -qx "atl-web-$i" || docker run -d --rm \
    --name "atl-web-$i" --network "${NETWORK}" \
...
# ---- Python helper: build strict payload & salvage JSON arrays ----
...
# ---- Call helper & API (supports sliced content override) ----
scan_one_payload() {
  # args: worker_idx file_path max_bytes max_tokens [content_override_path]
  ...
  # echo OK flag and the temp BODY path so caller can read $BODY.data
  echo "$OK" "$BODY"
}
...
run_chunk() {
  ...
  while IFS= read -r F; do
    ...
    if [[ "$OK" -ne 1 ]]; then
      echo "    ↳ 500 again → quartering + slicing file (first half)"
      SL_ALL="$(mktemp)"; nl -ba "$F" > "$SL_ALL"
      LINES=$(wc -l < "$SL_ALL"); HALF=$((LINES/2))
      SLICE="$(mktemp)"; head -n "$HALF" "$SL_ALL" > "$SLICE"
      read OK BODY <<< "$(scan_one_payload "$idx" "$F" "$((BASE_MAX_BYTES/4))" "$((BASE_MAX_TOKENS-96))" "$SLICE")"

      if [[ "$OK" -ne 1 ]]; then
        echo "    ↳ trying second half"
        tail -n +"$((HALF+1))" "$SL_ALL" > "$SLICE"
        read OK BODY <<< "$(scan_one_payload "$idx" "$F" "$((BASE_MAX_BYTES/4))" "$((BASE_MAX_TOKENS-96))" "$SLICE")"
      fi
    fi
    ...
    # Salvage array; attach path; write per-file + aggregate
    ARR="$(mktemp)"
...
# Merge JSONL → final pretty array
